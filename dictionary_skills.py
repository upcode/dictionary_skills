...
def remove_duplicates(words):
    ...
    # duplciate_words = to set(words) set method is unique itmes in list
    duplicate_words = set(words)
    # returns dupliate_words into a list
    return list(duplciate_words)
...
def sort_by_word_length(words):
    ...
    # created empty list
    my_list = []
    # using for loop to iterate over words
    for word in words:
    # using len to find leangth of words and appending word to list
        my_list.append((len(word), word))
        my_list.sort(reverse=True)
        res = my_list()
        for length, word in my_list:
            res.append(word)
    return res
...
def get_pirate_talk(phrase):
    ...
    list_phrase = pharase.split()
    word_list = []  # created empty list
    for word in list_phrase:  # iterate though word_list for word
        if word in priate_translation.append[word_list]:  # if word in priate append to word list
        else:
    return word_list
